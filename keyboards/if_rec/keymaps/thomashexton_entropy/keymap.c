/* IFKB Ergolite (via if_rec 2.4G receiver) — minimal Entropy/Vial build.
 *
 * This is a sibling of the plain `thomashexton` keymap. It keeps the same
 * compiled defaults and custom userspace behaviour while allowing the six
 * layers and four combo slots to be edited live.
 *
 ...
 *  KEYMAP HELPERS
 * ────────────────────────────────────────────────────────────────────────── */
#define THUMB_L_TAB   LT(LAYER_LOWER, KC_TAB)
#define THUMB_L_OUTER LT(LAYER_RAISE, KC_SPC)
#define THUMB_L_INNER OSM(MOD_LSFT)

#define THUMB_R_OUTER LT(LAYER_LOWER, KC_BSPC)
#define THUMB_R_INNER RSFT_T(KC_ENT)
...
/* Vial owns key_combos[] in this build. The four slots start empty and are
 * edited in Entropy. The plain sibling retains the compiled combo table. */
#ifndef VIAL_ENABLE
combo_t key_combos[] = {
    ALL_COMBOS
};
...
// Layer 3 is intentionally blank: LAYER_POINTER is shared with the Charybdis.
// clang-format off
